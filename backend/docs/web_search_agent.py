...
from ollama_status import check_ollama_health, OllamaStatus
...
async def get_ollama_client():
    ...
    if status.status == OllamaStatus.AVAILABLE:
        return AsyncOpenAI(
            base_url=f'{OLLAMA_BASE_URL}/v1',
            api_key='ollama'
        )
    raise ConnectionError(f"Ollama is not available: {status.error}")

client = None  # Will be initialized before use

model = None  # Will be initialized before use
